#!/bin/bash
# ═══════════════════════════════════════════════════════════════
# LexFlow — Server Setup Script
# DigitalOcean Ubuntu 24.04 LTS
# Execute: bash server-setup.sh
# ═══════════════════════════════════════════════════════════════
set -e

DOMAIN="app.seudominio.com.br"   # ← ALTERE AQUI
APP_DIR="/var/www/lexflow"
PHP_VERSION="8.3"
REPO="https://github.com/henrique-lopes/LexFlow.git"

echo "════════════════════════════════════════"
echo " LexFlow — Setup do servidor"
echo "════════════════════════════════════════"
...
cat > /etc/nginx/sites-available/lexflow <<NGINX
server {
    listen 80;
    server_name ${DOMAIN};
    root ${APP_DIR}/public;
...
cat > /etc/supervisor/conf.d/lexflow-worker.conf <<SUPERVISOR
[program:lexflow-worker]
